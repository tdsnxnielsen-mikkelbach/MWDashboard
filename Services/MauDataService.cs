...
public class MauDataService : IMauDataService
{
    ...
    public async Task<(DateTime? Earliest, DateTime? Latest)> GetLicenseDataRangeAsync()
    {
        ...
        // Use MauSnapshot ReportDate for actual historical range (Graph reports span up to 180 days)
        // LicenseSnapshot.CollectedAt only reflects when we stored data, not the data's time span
