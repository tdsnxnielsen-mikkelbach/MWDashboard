...
- **TTL 60 min** (with 20 min sliding): Licenses, message center, Copilot, segmentation, departments, Entra tiers, directory-audit/change-log, license-issues, oauth-grants, mailbox-access (daily-changing data)
...
- DbContext has 46 DbSets — all entities defined in `src/MWDashboard.Shared/Models/MauSnapshot.cs`
...
- **Bulk tenant collection** (`/tenants`): the "Collect All Tenants" button loops **active** tenants sequentially calling `IDataCollectionService.CollectForTenantAsync`, updating a live progress panel (current tenant, progress bar, per-tenant ✓/✗ log) via `StateHasChanged()` + `await Task.Yield()` between tenants, then shows an overall summary. Runs sequentially (not parallel) to avoid overwhelming the Collector container's HTTP scaling
