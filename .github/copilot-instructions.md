...
- `TenantInfo` tracks consent health: `MissingPermissions` (comma-separated Graph permissions that failed a consent probe; empty = all consented) + `PermissionsCheckedAt` (last probe time). Populated on every collection run. It also holds two Management-Activity cursors: `CopilotAuditCursorUtc` (unlicensed Copilot Chat) and `SharePointAuditCursorUtc` (external sharing)
- Governance snapshots (delete-then-insert per `(TenantId, ReportDate)`, all-tier): `AppCredentialSnapshot` (app secret/cert expiry; unique `(TenantId, ReportDate, AppObjectId, KeyId)`), `PrivilegedRoleSnapshot` (standing directory-role membership; unique `(TenantId, RoleName, ReportDate)`), `DefenderAlertSnapshot` (Defender alerts by severity/status; unique `(TenantId, Severity, Status, ReportDate)`). `ExternalSharingSnapshot` (upsert by `(TenantId, ShareType, ReportDate)`) is sourced from the Management Activity API, not Graph
...
- `ManagementActivityClient` is **content-type-generic** (audience `https://manage.office.com/.default`, per-tenant token cache, `Retry-After` backoff, AF20022/AF20024 handling): `EnsureSubscriptionAsync(tenantId, contentType, ct)` / `ListContentAsync(tenantId, contentType, startUtc, endUtc, ct)`. Two collectors reuse it: `CopilotAuditCollectionService` (`Audit.General` — filters `Workload=="Copilot"` + BizChat `AppHost` set, dedupes distinct users/day, splits licensed vs. unlicensed via `GraphReportService.GetCopilotLicensedUpnsAsync`, advances `TenantInfo.CopilotAuditCursorUtc`) and `ExternalSharingCollectionService` (`Audit.SharePoint` — `ClassifyShare` maps AnonymousLink→Anonymous, CompanyLink→Organization, guest sharing-invitation/secure-link ops→External; aggregates `ExternalSharingSnapshot` per (ShareType, day), advances `TenantInfo.SharePointAuditCursorUtc`). Both share the same cursor self-heal / 7-day-retention / 24h-window logic
...
- Web app polls it from `/copilot` ("Poll Copilot Chat") via `ICopilotAuditClient` → `HttpCopilotAuditClient` (typed HttpClient, `CopilotAuditBaseUrl`) with `LocalCopilotAuditClient` fallback when unconfigured. The same `/collect` call also refreshes external sharing (no separate endpoint/client — `IExternalSharingCollectionService` is registered in the CopilotAudit container, its cron, and the Web local-fallback DI)
...
- DbContext has 38 DbSets — all entities defined in `src/MWDashboard.Shared/Models/MauSnapshot.cs`
