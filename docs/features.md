...
A single page with seven tabs covering endpoint and identity governance. Each tab degrades gracefully with an info alert when its data/permission is unavailable, shows KPI cards plus charts, and adds a per-tenant comparison table in multi-tenant view.
...
- Data models: `DeviceComplianceSnapshot`, `ConditionalAccessSnapshot`, `GuestUserSnapshot`, `RiskyUserSnapshot`, `AppCredentialSnapshot`, `PrivilegedRoleSnapshot`, `OAuthGrantSnapshot` (one row per tenant per day; app credentials/privileged roles/OAuth grants delete-then-insert per tenant per day)
...
- **Collect All Tenants button** — runs collection across every **active** tenant sequentially without clicking each row. A live progress panel shows the current tenant being collected ("Collecting from *X*… (n of N)"), a progress bar, and a per-tenant confirmation list (green check on success, red error with the failure reason), followed by an overall summary ("completed for all N" or "n succeeded, m failed"). Disabled while any single-tenant collection is already running
