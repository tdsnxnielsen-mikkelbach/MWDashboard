...
- [x] **Legacy authentication & risky sign-in detail** — extend the existing sign-in summary with **legacy-auth protocol usage** (POP/IMAP/SMTP/older clients that bypass MFA — a concrete remediation the customer can act on) plus failed-sign-in / risky-sign-in and sign-in-by-country breakdowns. Builds directly on the data already pulled for `SecuritySignInSummary`. ✅ **Shipped** — `SignInDetailSnapshot` + `TenantInfo.SignInDetailCursorUtc` (cursor + ADD-on-upsert so split-day sign-ins accumulate beyond the tenant's ~30-day retention) + **Legacy Auth & Risky Sign-ins** section on `/security` (legacy-auth / failed / risky KPIs + per-client-app table with Legacy/Modern chips + top-15 sign-in-by-country table) + `signin-detail` export. P1/P2-gated (collected inside the existing `HasSignInAccess` block; `IsPremiumLicenseError` distinguishes a free-tier 403 from a consent gap).
  ...
  - Model: `SignInDetailSnapshot` (`TenantId`, `TenantName`, `ReportDate`, `ClientApp`, `IsLegacyAuth`, `Country`, `SuccessCount`, `FailureCount`, `RiskyCount`, `CollectedAt`); composite unique index `(TenantId, ClientApp, Country, ReportDate)` — the country dimension was added to the model so both the client-app legacy-auth breakdown and the sign-in-by-country view come from one table
  - Implementation steps:
    1. Model + DbSet + migration
