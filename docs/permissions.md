...
|-----------|---------|-------|
| `Reports.Read.All` | MAU usage reports, workload activity, user detail; M365 app per-user detail (`getM365AppUserDetail`), Office desktop activations (`getOffice365ActivationCounts` / `getOffice365ActivationsUserDetail`) and per-team Teams activity (`getTeamsTeamActivityDetail`) | Core functionality + Activity (incl. Top Teams) + Segmentation + M365 Apps pages. Per-user rows are anonymized at collection time (UPNs pseudonymized, names dropped) |
...
| `AuditLog.Read.All` | Entra sign-in logs; MFA/auth method registration (`/reports/authenticationMethods/userRegistrationDetails`); inactive-account `signInActivity`; legacy-auth / risky sign-in detail (`/auditLogs/signIns`); directory change-log / audit events (`/auditLogs/directoryAudits`) | Security page — sign-in logs, inactive accounts, and the **Legacy Auth & Risky Sign-ins** section require Entra ID P1/P2 on target tenant (the `signInActivity` property and sign-in logs are premium-gated); MFA registration and the **Change Log (Directory Audit)** section work on all tiers (free-tier directory audit retains only ~7 days, so the dashboard accumulates history in-DB over time) |
| `User.Read.All` | User department attribute; licensed/enabled member accounts & `signInActivity` for inactive-account analysis; per-SKU license assignment errors + disabled-but-licensed seat waste (`accountEnabled` + `licenseAssignmentStates`) | Department Adoption page (all tiers); Inactive Accounts on Security page (requires P1/P2 for `signInActivity`); **License Assignment Issues** section on Licenses page (all tiers) |
...
| `Application.Read.All` | App registration secret/certificate expiry inventory (`/applications` `passwordCredentials` + `keyCredentials`); OAuth app consent grants (`/servicePrincipals`, with `Directory.Read.All`) | Identity & Devices page — App Credential Expiry tab + **OAuth Apps** tab (all tiers); **new permission — requires re-consent** |
...
| `SecurityAlert.Read.All` | Microsoft Defender / Microsoft 365 security alerts by severity & status (`/security/alerts_v2`); Defender for Office 365 email-threat detections (same endpoint, filtered to email categories) | Security page — Defender Alerts section; Threat Protection page — **Email Threats** tab (requires a Defender for Office 365 / EOP plan, MDO P1/P2); requires a Microsoft Defender / Defender XDR subscription on the tenant; **new permission — requires re-consent** |
| `AttackSimulation.Read.All` | Attack Simulation Training campaigns + per-campaign report overview (`/security/attackSimulation/simulations` with `$expand=report`) — targeted/clicked/reported funnel + compromised rate | Threat Protection page — **Attack Simulation Training** tab; **requires Defender for Office 365 Plan 2** on target tenant; **new permission** — intentionally **not** in the consent probe (P2-licensing-gated) |
| `Directory.Read.All` | Commercial subscription renewal/expiry dates (`/directory/subscriptions` — `nextLifecycleDateTime`, `status`, `isTrial`, seat counts); OAuth app consent grants (`/oauth2PermissionGrants` + `/servicePrincipals`); stale Entra-registered devices (`/devices` — `operatingSystem`, `approximateLastSignInDateTime`, `accountEnabled`) | Licenses page — Subscription Renewals & Expiry section (all tiers); Identity & Devices page — **OAuth Apps** tab (with `Application.Read.All`; all tiers) + **Stale Registered Devices** tab (all tiers) |
...
|-----------|----------|---------|-------|
| `ActivityFeed.Read` | **Office 365 Management APIs** (not Microsoft Graph) | Read raw audit events from four content types: `Audit.General` (`CopilotInteraction`) to measure free, unlicensed Copilot Chat usage; `Audit.SharePoint` (sharing operations) to measure external/anonymous file-sharing activity; `Audit.Exchange` (`New-InboxRule` / `Set-InboxRule` / `Set-Mailbox`) to detect suspicious mailbox forwarding/redirect/delete rules (a BEC indicator) **and** non-owner mailbox access / delegate grants (`MailItemsAccessed` / `Add-MailboxPermission`, an insider-risk indicator); and `DLP.All` to surface Purview DLP policy matches — none exposed by the Graph reports API | Granted by the same admin-consent flow (the `/adminconsent` URL grants every permission configured on the app registration), but it must first be **added to the app registration** under *Office 365 Management APIs → Application permissions*. Requires **unified audit logging** to be enabled on the tenant. Drives the unlicensed Copilot Chat metric, the External Sharing + Suspicious Mailbox Rules + Mailbox Access + DLP Policy Matches sections on the Security page. Intentionally **not** part of the Graph consent probe (the probe only exercises Graph endpoints). |
...
|----------------------|-----------------|
| Entra ID Free | MAU reports, licenses, subscription renewals, license assignment issues, Message Center, workload activity, Teams team activity, segmentation, departments, MFA registration, Secure Score, Service Health, device compliance, stale registered devices, guest users, app credential expiry, OAuth app consent grants, privileged-role inventory, directory change-log (~7-day native retention, accumulated in-DB over time), Defender alerts (where a Defender subscription exists), external sharing, suspicious mailbox rules, mailbox access, DLP matches (where Purview DLP is configured) — **no sign-in logs, no inactive-account analysis, no Conditional Access policies, no risky-user analysis** (`signInActivity`, Conditional Access, and Identity Protection are premium-gated) |
...
> The dashboard detects each tenant's tier from its license SKUs (`TenantEntraTier.FromLicenses`) and **skips** premium-gated collection on tenants that lack the required tier — sign-in summary, legacy-auth / risky sign-in detail, and inactive accounts on free-tier tenants, and risky-user analysis on anything below P2 — logged at Information rather than as a permission error. The Tenants page shows a per-tenant **Plan** chip (Free/P1/P2) so this limitation is visible at a glance. Because `IdentityRiskyUser.Read.All` is P2-gated, it is intentionally **not** included in the consent probe (a 403 on a non-P2 tenant is a licensing limit, not a consent gap, and would otherwise produce a false re-consent flag).

> **Defender for Office 365 licensing:** email-threat detections (Email Threats tab) require a Defender for Office 365 / EOP plan (**MDO P1 or P2**) and Attack Simulation Training (Attack Simulation tab) requires **MDO Plan 2**. The dashboard derives a separate Defender tier from license SKUs (`TenantDefenderTier.FromLicenses`) and **skips** these collections on under-licensed tenants (logged at Information). The Tenants page shows a separate per-tenant **Defender** chip (MDO P1 / MDO P2 / Not licensed). Like `IdentityRiskyUser.Read.All`, `AttackSimulation.Read.All` is intentionally **not** in the consent probe (its 403 on a non-P2 tenant is a licensing limit, not a consent gap). Email threats reuse the already-granted `SecurityAlert.Read.All`, so no extra consent is needed for that tab.
