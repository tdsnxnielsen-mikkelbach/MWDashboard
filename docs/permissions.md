...
| `Directory.Read.All` | Commercial subscription renewal/expiry dates (`/directory/subscriptions` — `nextLifecycleDateTime`, `status`, `isTrial`, seat counts); OAuth app consent grants (`/oauth2PermissionGrants` + `/servicePrincipals`) | Licenses page — Subscription Renewals & Expiry section (all tiers); Identity & Devices page — **OAuth Apps** tab (with `Application.Read.All`; all tiers) |
